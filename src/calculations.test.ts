	...
	describe("Core Algorithm Validation", () => {
		it("should calculate reasonable burn times for Type I skin without sunscreen", () => {
			...
			const burnTimeMinutes = result.burnTime
				? (result.burnTime.getTime() - input.currentTime.getTime()) /
					(1000 * 60)
				: 0;
	...
	describe("UV Index Sensitivity", () => {
		it("should burn faster in high UV conditions", () => {
			...
			const highUVBurnTime = highUVResult.burnTime
				? (highUVResult.burnTime.getTime() - highUV.currentTime.getTime()) /
					(1000 * 60)
				: Infinity;
			const lowUVBurnTime = lowUVResult.burnTime
				? (lowUVResult.burnTime.getTime() - lowUV.currentTime.getTime()) /
					(1000 * 60)
				: Infinity;
	...
	describe("SPF Protection Levels", () => {
		it("should provide appropriate protection for SPF 15", () => {
			...
			const noSPFTime = noSPFResult.burnTime
				? (noSPFResult.burnTime.getTime() - withoutSPF.currentTime.getTime()) /
					(1000 * 60)
				: 0;
			const spfTime = spfResult.burnTime
				? (spfResult.burnTime.getTime() - withSPF.currentTime.getTime()) /
					(1000 * 60)
				: 0;
	...
	describe("Sweat Level Degradation", () => {
		it("should reduce SPF effectiveness over time with sweating", () => {
			...
			const noSweatTime = noSweatResult.burnTime
				? (noSweatResult.burnTime.getTime() - noSweat.currentTime.getTime()) /
					(1000 * 60 * 60)
				: Infinity;
			const highSweatTime = highSweatResult.burnTime
				? (highSweatResult.burnTime.getTime() -
						highSweat.currentTime.getTime()) /
					(1000 * 60 * 60)
				: Infinity;
			...
			if (noSweatTime === Infinity && highSweatTime === Infinity) {
				...
				const noSweatDamage =
					noSweatResult.points[noSweatResult.points.length - 1]
						?.totalDamageAtStart || 0;
				const highSweatDamage =
					highSweatResult.points[highSweatResult.points.length - 1]
						?.totalDamageAtStart || 0;
	...
	describe("Edge Cases and Error Handling", () => {
		...
		it("should provide conservative estimates for safety", () => {
			...
			if (lastPoint && result.burnTime) {
				expect(
					lastPoint.totalDamageAtStart + lastPoint.burnCost,
