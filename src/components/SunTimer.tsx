...
export function SunTimer({ result }: SunTimerProps) {
  ...
  const calculateRealTimeDamage = useCallback((elapsedMs: number, startTime: Date) => {
    ...
    for (const point of result.points) {
      if (point.slice.datetime <= currentTime) {
        ...
        if (prevPoint && prevPoint.slice.datetime <= startTime) {
          const sliceDuration = point.slice.datetime.getTime() - prevPoint.slice.datetime.getTime()
          const elapsedInSlice = currentTime.getTime() - prevPoint.slice.datetime.getTime()
          const partialDamage = (elapsedInSlice / sliceDuration) * point.burnCost
    ...
    if (timer.isRunning && timer.startTime) {
      interval = setInterval(() => {
        setTimer(prev => {
          ...
          const newDamage = calculateRealTimeDamage(newElapsedMs, prev.startTime!)
  ...
  const getRiskStatus = (damage: number) => {
    if (damage < 25) return { status: 'Safe', color: 'bg-green-500', textColor: 'text-green-700' }
    if (damage < 50) return { status: 'Caution', color: 'bg-yellow-500', textColor: 'text-yellow-700' }
    if (damage < 75) return { status: 'Warning', color: 'bg-orange-500', textColor: 'text-orange-700' }
    if (damage < 95) return { status: 'Danger', color: 'bg-red-500', textColor: 'text-red-700' }
    return { status: 'Critical', color: 'bg-red-600', textColor: 'text-red-800' }
  }

  const riskStatus = getRiskStatus(timer.accumulatedDamage)
  const burnTime = result.burnTime
  const remainingTime = burnTime && timer.startTime ? 
    Math.max(0, burnTime.getTime() - (timer.startTime.getTime() + timer.elapsedMs)) : null

  return (
    <Card className="bg-stone-100 border-stone-200 shadow-sm">
      <CardHeader>
        <CardTitle className="flex items-center justify-between text-slate-800">
          ...
          <Badge variant={timer.accumulatedDamage > 75 ? "destructive" : "secondary"}>
            {riskStatus.status}
          </Badge>
        </CardTitle>
      </CardHeader>
            ...
            <div className="flex items-start space-x-2">
              ...
              <div className="text-sm">
                ...
                <p className="text-green-700">
                  Your current sun exposure is within safe limits. Enjoy your time outdoors!
                </p>
              </div>
            </div>
          </div>
