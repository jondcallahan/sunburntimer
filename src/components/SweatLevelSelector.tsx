...
function SweatLevelOption({ level, selected, onSelect }: SweatLevelOptionProps) {
  const config = SWEAT_CONFIG[level]
  
  const getDropletCount = (level: SweatLevel) => {
    switch (level) {
      case SweatLevel.LOW: return 0
      case SweatLevel.MEDIUM: return 1
      case SweatLevel.HIGH: return 3
    }
  }
                  ...
                  <Droplets
                    ...
                    className={`w-3 h-3 ${
                      i < dropletCount
                        ...
                        : selected ? 'text-primary-foreground/30' : 'text-muted-foreground/30'
                    }`}
                  />
                ))}
              </div>
  ...
  return (
    <div className="space-y-4">
      <p className="text-sm text-muted-foreground">
        How much will you be sweating? This affects how quickly your sunscreen wears off.
      </p>
      ...
      {sweatLevel && sweatLevel !== SweatLevel.LOW && (
        <Alert>
          ...
          <AlertDescription>
            <strong>Note:</strong> Your sunscreen protection will gradually decrease due to sweating. 
            Consider reapplying more frequently.
          </AlertDescription>
        </Alert>
      )}
    </div>
