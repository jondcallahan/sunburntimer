...
export function useLocationRefresh() {
    ...
    if (
      ...
      const refreshWeather = async () => {
        try {
          ...
          setGeolocationError(
            error instanceof Error ? error.message : 'Failed to refresh weather data'
          )
        }
      }
  ...
  }, [geolocation.status, geolocation.position, geolocation.weather, setGeolocationStatus, setWeather, setGeolocationError])
}