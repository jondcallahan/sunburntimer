			...
			(error) => {
				const errorMessages: Record<number, string> = {
					[error.PERMISSION_DENIED]:
						"This site is not allowed to use your location. Please enable location permissions in your browser settings.",
					[error.POSITION_UNAVAILABLE]:
						"Your device failed to discover your location. Please check that location services are enabled.",
					[error.TIMEOUT]:
						"Your device could not determine your position in a reasonable time. Please try again.",
				};
				reject(
					new Error(
						errorMessages[error.code] ||
							"Your device failed to discover your location due to an unknown error.",
					),
				);
			},
			options,
...
export async function reverseGeocode(
	position: Position,
): Promise<{ placeName: string; countryCode: string }> {
	try {
